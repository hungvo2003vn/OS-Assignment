...
#define PAGING_MAX_PGN  (DIV_ROUND_UP(BIT(PAGING_CPU_BUS_WIDTH),PAGING_PAGESZ)) //fix 
...
//int enlist_pgn_node(struct pgn_t **pgnlist, int pgn); //old version
int enlist_pgn_node(struct memphy_struct *mp, int pgn, uint32_t *pgd_pgn); //modified
